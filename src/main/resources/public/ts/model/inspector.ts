...
export class InspectorHabilitation {
    ...
    async create () {
        ...
        return ToastUtils.setToastMessage(response, 'inspector.habilitation.created','inspector.habilitation.created.error');
    }

    async delete() {
        let response = await http.delete(`/diary/inspector-habilitation/${this.inspector.id}/${this.teacher.id}/${this.structure.id}`);
        return ToastUtils.setToastMessage(response, 'inspector.habilitation.deleted','inspector.habilitation.deleted.error');
    }
